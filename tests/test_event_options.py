...
def test_admin_cannot_delete_event_option_in_use(admin_client, app, guest):
    option = _make_option()
    guest.event_option_id = option.id
    db.session.commit()
...
def test_guest_details_without_options_has_no_date_field(client, guest):
    ...
    assert b"event_option_id" not in response.data
...
def test_guest_must_choose_a_date_when_options_exist(client, guest, app):
    ...
    assert response.status_code == 200
    assert "Merci de choisir une date".encode() in response.data

    refreshed = Guest.query.filter_by(token=guest.token).first()
    assert refreshed.event_option_id is None


def test_guest_can_choose_a_date(client, guest, app):
    option = _make_option(label="Samedi")
    client.post(f"/rsvp/{guest.token}/confirmer")

    response = client.post(
        f"/rsvp/{guest.token}/details",
        data={"dietary_notes": "", "event_option_id": str(option.id)},
    )
    assert response.status_code == 302

    refreshed = Guest.query.filter_by(token=guest.token).first()
    assert refreshed.event_option_id == option.id
